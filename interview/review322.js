...
var thoundsSplit = function (s) {
  let arr = s.split(".");
  return arr[0].replace("/dd{3}$/g", function () {}).concat(arr[1]);
};
...
var isEqual = function (obj, target) {
  ...
  if (typeof obj === "object" && typeof target === "object" && obj && target) {
    if (obj.keys.length !== target.keys.length) return false;
    for (let key in obj) {
...
var isEqual = function (x, y) {
  ...
  for (let key in x) {
    if (!y.hasOwnerProperty(key)) return false;
...
var promiseAll = function (promises) {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (var i = 0; i < promises.length; ++i) {
      (function (j) {
        Promise.resolve(promises[j]).then(
          (v) => {
            count++;
            res[j] = v;
